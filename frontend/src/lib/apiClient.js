...
const suspendUser = (userId, isSuspended) =>
  apiClient.patch(`api/v1/users/${userId}`, {
    is_suspended: isSuspended
  });

const adminEditUser = (data, userId) =>
  apiClient.patch(`api/v1/users/${userId}`, {
...
const getRecipes = search => {
  ...
  return apiClient.get(`api/v1/recipes${query}`).then(results => results.data);
};
...
const requestPasswordReset = email =>
  apiClient.post('/api/v1/password', {
    user: {
      email
    }
  });
